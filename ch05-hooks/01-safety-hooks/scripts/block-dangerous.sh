...
# --- Catastrophic / unauthorized deletion ---
# rm targeting root '/' is never allowed.
if echo "$CMD" | grep -qE '\brm[[:space:]]+(-[a-zA-Z]+[[:space:]]+)*/($|[[:space:]])'; then
  block "rm targeting root '/' is never allowed."
fi
# Recursive-force rm (-rf, -fr, -Rf, ...) against ANY target — relative OR
# absolute path. Only /tmp, /var/tmp and ~/.cache are whitelisted.
if echo "$CMD" | grep -qE '\brm[[:space:]]+-[a-zA-Z]*([rR][a-zA-Z]*[fF]|[fF][a-zA-Z]*[rR])[a-zA-Z]*[[:space:]]+[^[:space:]]'; then
  TARGET=$(echo "$CMD" | grep -oE 'rm[[:space:]]+-[a-zA-Z]+[[:space:]]+\S+' | awk '{print $NF}')
  case "$TARGET" in
    /tmp/*|/var/tmp/*|"$HOME"/.cache/*) : ;;
    *) block "rm -rf is restricted to /tmp or ~/.cache in this workshop. Target was: $TARGET" ;;
  esac
fi
# rmdir — only removes empty dirs, but the workshop treats every directory
# removal as a guarded action. Same whitelist as rm -rf.
if echo "$CMD" | grep -qE '\brmdir([[:space:]]|$)'; then
  TARGET=$(echo "$CMD" | grep -oE 'rmdir([[:space:]]+-[a-zA-Z-]+)*[[:space:]]+\S+' | awk '{print $NF}')
  case "$TARGET" in
    /tmp/*|/var/tmp/*|"$HOME"/.cache/*) : ;;
    *) block "rmdir is restricted to /tmp or ~/.cache in this workshop. Target was: $TARGET" ;;
  esac
fi
